...
all_zip <- house_data_og %>% 
  mutate(zip = as.character(zip)) %>% 
  right_join(y = all_zip, by = "zip")
```
...
if (params$refit_model1) {
  ...
  fit_time_lin2 <- update(
    fit_time_lin1,
    start = ss,
    control = glmerControl(optimizer = "bobyqa",boptCtrl = list(maxfun = 2e6))
  )
  toc()
...
```{r missingness-model}
all_pcr$race_unknown <- factor(
  ifelse(all_pcr$race == "unknown", "unknown", "known"),
  levels = c("known", "unknown")
)

if (params$refit_model_miss) {
  tic()
  miss_model <- glm(
    formula = race_unknown ~ covid_positive + age_group + sex + 
              adj_perc_bach_quar + adj_perc_insured_quar +
              adj_pop_density + adj_med_income +
              time_days,              
    family = binomial, 
  ...
  skrrrahh("drummaboy")
#  sec elapsed
# updated and saved 2021-05-25
  save(miss_model, file = here("analysis/testing-positive-regression-results", "miss_model.Rdata"))
} else {
  load(file = here("analysis/testing-positive-regression-results", "miss_model.Rdata"))
}


miss_sum <- summary(miss_model)
miss_sum_df <- data.frame(miss_sum$coefficients)
miss_model_est <- data.frame(
  "odds" = exp(miss_sum_df$Estimate), 
  "lb95" = exp(miss_sum_df$Estimate - qnorm(0.975) * miss_sum_df$Std..Error), 
  "up95" = exp(miss_sum_df$Estimate + qnorm(0.975) * miss_sum_df$Std..Error),
  "pval" = miss_sum_df$Pr...z..
)
rownames(miss_model_est) <- rownames(miss_sum$coefficients)
miss_model_est
```
