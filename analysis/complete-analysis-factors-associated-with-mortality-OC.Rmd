...
For simplicity let  
$p_{i}$ being the probability of mortality among individuals who tested positive for COVID-19 in Orange County.  
...
if (params$refit_model1) {
  tic()
  fit0 <- glm(
    formula = death_due_to_covid  ~ age + sex + race + 
              adj_perc_insured_quar +
  ...
  #save(fit_time_lin_m, file = here("analysis/mortality-regression-results", "fit_time_lin_m.Rdata"))
} else {
  #load(file = here("analysis/mortality-regression-results", "fit_time_lin_m.Rdata"))
}
```


```{r fit0-results} 
compute_ci_logistic <- function(model, alpha = 0.05, param_names) {
  #does not include intercept or median income
  model_summary <- summary(model)
  in_med_inc <- length(model_summary$p.coeff)
  coeffs <- model_summary$p.coeff[-1]
...
\begin{equation}
  \label{eq:Model1}
  \begin{split}
    \log(\frac{p_i}{1 + p_i}) &= \beta_0 + \beta^{T}_{1}\text{Age}_i + \overrightarrow{\beta}_{2}\text{Male}_i + \overrightarrow{\beta}_{(3-9)}\overrightarrow{\text{Race}}_i\\
    &+ \overrightarrow{\beta}_{(10-12)}\overrightarrow{\text{College Degree}}_i + \overrightarrow{\beta}_{(23-15)}\overrightarrow{\text{Insured}}_i + \beta_{27}\text{PopDensity}_i\\
    &+ \beta_{16}\text{MedianIncome}_i\\ 
    &+ \beta_{17}\text{Days}_i,
  \end{split}
\end{equation}
...
if (params$refit_model1) {
  ...
  tic()
  fit_time_lin_m2 <- allFit(fit_time_lin_m)
  toc()
  skrrrahh("drummaboy")
...
if (params$refit_model4) {
  ...
  toc()
  skrrrahh("drummaboy")
  save(fit_time_gam_m, file = here("analysis/mortality-regression-results", "fit_time_gam_m.Rdata"))
} else {
  load(file = here("analysis/mortality-regression-results", "fit_time_gam_m.Rdata"))
}
```
