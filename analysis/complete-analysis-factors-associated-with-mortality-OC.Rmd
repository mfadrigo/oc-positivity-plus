...
tab_obs <- data.frame(matrix( data = NA, ncol = 2, nrow = 3,))
rownames(tab_obs) <- c(
  "Unmodified",
  "Nonresponse[note]",
  "Invalid zip code[note]"
)
tab_obs[1, 1] <- pos_counts["full_num_data_cases"]
tab_obs[1, 2] <- NA
tab_obs[2:3, 1] <- pos_counts[-1]
...
kable(
  ...
  add_footnote(
    c(
      "Number of tests without complete demographic (sex, age, zip) information",
      "Number of invalid zip codes and non Orange County zip codes"
    ),
    notation = "symbol"
  )
```
