...
read_pos_pcr <- function(file_path, start_date, end_date) {
  ...
  # pos_results_original_new_race$race1[pos_results_original_new_race$race1 == "other"] <- "unknown"
...
\begin{equation}
  \label{eq:Model0}
  \begin{split}
    \log(O_i) &= \beta_0 + \beta_{1}\text{Age}_i + \beta_{2}\text{Sex}_i + \overrightarrow{\beta}_{(3-9)}\overrightarrow{\text{Race}}_i\\
		...
		&+ \beta_{16}\text{Population Density}_i + \beta_{17}\text{Median Income}_i\\ 
		&+ \beta_{18}\text{Days}_i\\
		&+ \beta_{19}\text{Available Hospital Beds}_i,
  \end{split}
\end{equation}
without a random intercept for zip code.

```{r fit-model0, eval = FALSE}
...
```{r final-model-results-table, eval = FALSE}
# Not ready yet!
...
intercept <- paste0(
  round(exp(fm_sum$coefficients[, 1]), 3),
  " (",
  round(exp(fm_sum$coefficients[, 1] - qnorm(0.975) * sqrt(diag(sandwich(fit0)))[1]), 2),
  ",",
  round(exp(fm_sum$coefficients[, 1] + qnorm(0.975) * sqrt(diag(sandwich(fit0)))[1]), 2),
  ")"
)
