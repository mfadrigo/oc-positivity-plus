---
title: "Complete analysis: Factors associated with mortality among people diagnosed
  with SARS-CoV2"
output: 
  pdf_document:
    extra_dependencies:
...
read_pos_pcr <- function(file_path, start_date, end_date) {
  ...
  hos_bed_gov <- hos_bed_gov %>% 
    ...
    mutate(perc_avail_beds = ifelse(
      ...
      1 - params$oc_hos_covid_pateients_earliest_val / params$oc_all_hos_bed_earliest_val,
      1 - hospitalized_covid_patients / all_hospital_beds
    )) %>% 
    select(
  ...
  pos_results_merged$adj_avail_icu_beds <- scale(
    pos_results_merged$avail_icu_beds, 
    center = TRUE, 
    scale = TRUE
  )
  ...
  # Scale age ----------------------------------------------------------------------------------------------
  pos_results_merged$adj_age <- scale(
    pos_results_merged$age, 
    center = TRUE, 
    scale = TRUE
  )
...
parameter_names <- c(
  ...
  " 2nd Quartile", " 3rd Quartile", " 4th Quartile",
  "Population Density", 
  "Median Income (sd. dev)",
  "Time (Days)",
  "Available Beds (sd. dev)"
  )
...
rownames(table_fm) <- c(
  ...
  "Adjusted Median Income (std dev.)",
  "Adjusted Time (Days)",
  "Available Beds (std dev.)"
  )
...
quant_vars <- c("decades_old", "adj_pop_density", "adj_med_income", "adj_time_days", "adj_avail_beds")
...
```{r final-model-results-plot, fig.align="center", fig.cap="."}
...
fm_results_with_ref <- rbind(
  ...
  summary_fit0[10:nrow(summary_fit0), ]
)
...
data.frame(
(exp(fit3_sum$coefficients[, 1])),
(exp(fit3_sum$coefficients[, 1] - qnorm(0.975) * fit3_sum$coefficients[, 2])),
(exp(fit3_sum$coefficients[, 1] + qnorm(0.975) * fit3_sum$coefficients[, 2])),
)


plot(hos_bed_gov$test_date, hos_bed_gov$avail_beds)
```
