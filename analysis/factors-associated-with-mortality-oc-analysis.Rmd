...
kable(compare_bic,
      ...
      format = "latex",
      caption = "Model comparison using BIC shows do real difference in modeling odds of mortality given tested positive for SARS-CoV2 in Orange County. Therefore the simpler model, Model 0, was chosen.") %>%
...
parameter_names <- c(
  ...
  "Time (std. dev.)",
  "ICU COVID patients[note] (std. dev.)"
)
...
colnames(table_fm) <- c("Deaths[note]", "Total", "with (95% CI[note])")
rownames(table_fm) <- c(
  "Age (decades)",
  "Female", "Male",
...
kable(
  ...
  add_footnote(
    c(
      ...
      "All deaths with cause of death on death certificate specified as related to COVID-19",
...
ggplot(fm_results_with_ref, aes(x = param_names, y = odds), size = 5, color = variable_colors) +
  ...
  ggtitle("Results of SARS-CoV2 related mortality given SARS-CoV2 positive logisitic regression")
```
