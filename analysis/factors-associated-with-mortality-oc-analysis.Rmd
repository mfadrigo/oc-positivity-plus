...
read_pos_pcr <- function(file_path, start_date, end_date) {
  ...
  ids_of_deaths <- mortality_og %>% # if a person every died of covid it is recorded under each of their tests
    filter(DeathDueCOVID == "y") %>% 
    pull(unique_num)
    
  mortality_cleaned <- mortality_og %>% 
    mutate(covid_death = ifelse(unique_num %in% ids_of_deaths, "yes", "no")) %>% 
    select(id = unique_num, time_days, covid_death, death_date, decades_old) %>% 
    arrange(id) %>% 
  ...
  all_pcr <- data.frame(all_pcr_and_zip[["pcr_results_merged"]]) %>% 
    select(-c(time_days, adj_time_days))

  all_pcr_with_deaths <- mortality_cleaned %>% 
    right_join(y = all_pcr, by = "id") %>% 
    mutate(zip = droplevels(zip)) # drop zip = 92678 b/c there are no observations
  
  pos_results_adjusted <- all_pcr_with_deaths %>% 
  ...
  list(
    "all_results_merged" = all_pcr_with_deaths,
    "pos_results_merged" = pos_results_merged, 
