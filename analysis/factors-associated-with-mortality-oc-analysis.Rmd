...
read_pos_pcr <- function(file_path, start_date, end_date) {
  ...
  pos_results_adjusted <- mortality_og %>% 
    mutate(DeathDueCOVID = replace_na(DeathDueCOVID, "n")) %>% 
    mutate(death_due_to_covid = factor(str_to_lower(DeathDueCOVID))) %>% 
    select(id = unique_num, death_due_to_covid, time_days, decades_old, death_date) %>% 
    right_join(y = all_pos) %>% 
...
if (params$reclean_data) {
  pos_pcr_and_zip <- read_pos_pcr(
    file_path = here("data/mortality-data", "1.4.21 release to UCI team.csv"),
