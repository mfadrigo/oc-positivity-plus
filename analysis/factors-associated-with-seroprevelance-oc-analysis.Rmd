...
\label{eq:Model3}
	\begin{split}
		\log(O_i) &= \beta_0 + \beta_{\text{\% of Zip Code SARS-CoV2+}}\text{\% of Zip Code SARS-CoV2+}_i,
	\end{split}
\end{equation}
without a random intercept for zip code.
...
summary_fit2 <- compute_ci_logistic_glm(fit2, param_names = c(parameter_names, "% of Zip SARS-CoV2+ (10%)"))
summary_fit3 <- compute_ci_logistic_glm(fit3, param_names = "% of Zip SARS-CoV2+ (10%)")
...
row.names(compare_bic) <- c("Model 0", "Model 1", "Model 2", "Model3", "Model4")
...
kable(
  ...
  format = "latex", 
  caption = paste(
    "Model 2 regression estimation of adjusted odds ratio of testing sero-positive for SARS-CoV2 in Orange County.", 
    sep = ""
    )
  ) %>%
  kable_styling(latex_options = c("HOLD_position"), font_size = 11) %>%
...
```{r fit2-model-results-plot, fig.align="center", fig.cap="Logistic model 2 results for odds of testing sero-positive for SARS-CoV2 in Orange County. Pasific Islander adjusted odds ratio omitted due to width of confidence interval (1.05, 15.14)."}
...
ggplot(fm_results_with_ref, aes(x = param_names, y = odds), size = 5, color = variable_colors) +
  ...
  scale_x_discrete(limits = rev(fm_results_with_ref$param_names)) +
  scale_y_continuous(breaks = seq(0, max(fm_results_with_ref$upper_bound), by = 0.5)) +
  ggtitle("Results of SARS-CoV2 seropositivity logisitic regression (with incidence)")
```


```{r fit3-model-results-plot, fig.align="center", fig.cap="Logistic model 3 results for odds of testing sero-positive for SARS-CoV2 in Orange County modeled only by percent of their zip code that has tested positive for SARS-CoV2"}
...
```{r investigate-percent-pos-relationship, eval=FALSE}
modeled_vars <- all_sero[, c("perc_zip_covid_pos", "age_grp", "sex", "race", "adj_perc_bach_quar", "adj_perc_insured_quar", "pop_density", "med_income")]

pdf(file = here("report", "modeled-predictors.pdf"))
plot(modeled_vars)
dev.off()

modeled_zip_vars <- all_sero[, c("perc_zip_covid_pos", "adj_perc_bach_quar", "adj_perc_insured_quar", "pop_density", "med_income")]

pdf(file = here("report", "modeled-zip-code-variables.pdf"))
plot(modeled_zip_vars)
dev.off()
...
kable(
  ...
  format = "latex", 
  caption = paste(
    "Model 4 regression estimation of adjusted odds ratio of testing sero-positive for SARS-CoV2 in Orange County.", 
    sep = ""
    )
  ) %>%
  kable_styling(latex_options = c("HOLD_position"), font_size = 11) %>%
...
```{r fit4-model-results-plot, fig.align="center", fig.cap="Logistic model 4 results for odds of testing sero-positive for SARS-CoV2 in Orange County. Pasific Islander adjusted odds ratio omitted due to width of confidence interval (1.05, 15.14)."}
...
\newpage
```{r invesitgate-incidence-by-zip}
inc_by_his_seropos <- all_sero %>% 
  group_by(name) %>% 
  summarize(
    num_hispanic_seropos = sum(race == "hispanic" & covid_pos == "yes"),
    num_seropos = sum(covid_pos == "yes"),
    num_hispanic = sum(race == "hispanic"),
    tot_tested_in_zip = n()
  ) %>% 
  mutate(per_hispanic_seropos = round(100*num_hispanic_seropos / num_hispanic), 2) %>% 
  mutate(per_seropos = round(100*num_seropos / tot_tested_in_zip), 2) %>% 
  select(
    name,
    num_hispanic_seropos,
    per_hispanic_seropos,
    per_seropos,
    tot_tested_in_zip
  )
