...
kable(compare_bic,
      ...
      format = "latex",
      caption = "Model comparison using BIC shows do real difference in modeling odds of testing sero-positive for COVID-19 in Orange County. Therefore the simpler model, Model 0, was chosen.") %>%
...
qual_vars_ordered <- c(
  ...
  "age_grp.40-44", "age_grp.45-49", "age_grp.50-54", "age_grp.55-59",
  "age_grp.60-64", "age_grp.65-69", "age_grp.70-74", "age_grp.75-79",
  "age_grp.80-84", "age_grp.85+",
  "sex.female", "sex.male", 
  "race.white", "race.asian", "race.black", "race.hispanic", 
  "race.native", "race.islander", "race.unknown",
...
# No sero-positive native americans or native alaskans
table_fm["Native American", 1] <- paste(0, " (", 0, "%)", sep = "")
...
kable(
  ...
  pack_rows("Age", 1, 14, bold = FALSE) %>% 
  pack_rows("Sex", 15, 16, bold = FALSE) %>%
  pack_rows("Race", 17, 23, bold = FALSE) %>%
  pack_rows("% with College Degree[note]", 24, 26, bold = FALSE) %>%
  pack_rows("% with Insurance", 27, 29, bold = FALSE) %>% 
...
```{r fit0-model-results-plot, fig.align="center", fig.cap="Logistic model 0 results for odds of testing sero-positive for SARS-CoV2 in Orange County."}

fm_results_with_ref <- rbind(
  c(1, 0.99, 1.01, paste0(c("Age", rep(" ", 34), "[ref: 18-24]"), collapse = "")),
  summary_fit0[1:13, ],
  c(1, 0.99, 1.01, paste0(c("Sex", rep(" ", 38), "[ref: Female]"), collapse = "")),
  summary_fit0[14, ],
  c(1, 0.99, 1.01, paste0(c("Race", rep(" ", 24), "[ref: White]"), collapse = "")),
  summary_fit0[15:20, ],
  c(1, 0.99, 1.01, "% with College Degree [ref: 1st Quartile]"),
  summary_fit0[21:23, ],
  c(1, 0.99, 1.01, paste0(c("% with Insurance", rep(" ", 10), "[ref: 1st Quartile]"), collapse = "")),
  summary_fit0[24:nrow(summary_fit0), ]
)
...
ggplot(fm_results_with_ref, aes(x = param_names, y = odds), size = 5, color = variable_colors) +
  ...
  scale_y_continuous(breaks = seq(0, max(fm_results_with_ref$upper_bound), by = 1))


fm_results_with_ref$upper_bound[22] <- max(fm_results_with_ref$upper_bound[-22])
...
ggplot(fm_results_with_ref, aes(x = param_names, y = odds), size = 5, color = variable_colors) +
  ...
  scale_y_continuous(
    ...
    limits = c(min(fm_results_with_ref$lower_bound[-22]), max(fm_results_with_ref$upper_bound[-22]))
  )
```