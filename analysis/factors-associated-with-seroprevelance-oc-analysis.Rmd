...
kable(
  ...
  pack_rows("Race", 12, 18, bold = FALSE) %>%
  pack_rows("% with College Degree[note]", 19, 22, bold = FALSE) %>%
  pack_rows("% with Insurance", 23, 25, bold = FALSE) %>% 
...
fm_results_with_ref <- rbind(
  ...
  summary_fit0[19:nrow(summary_fit0), ]
)
...
fm_results_with_ref$upper_bound[17] <- max(fm_results_with_ref$upper_bound[-17])
...
ggplot(fm_results_with_ref, aes(x = param_names, y = odds), size = 5, color = variable_colors) +
  ...
  scale_y_continuous(
    ...
    limits = c(min(fm_results_with_ref$lower_bound[-17]), max(fm_results_with_ref$upper_bound[-17]))
  )
```