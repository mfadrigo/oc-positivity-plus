...
colnames(table_m1) <- c("Counts", "Adjusted odds[note] (95% CI[note])")
...
for(i in 1:nrow(table_m1)) {
  if (reference_group[i]) {
    ...
    table_m1[i, 2] <- paste(rr_m1[non_ref_counter], " (", 
...
levels(all_pcr$race)
variable_counts <- rep(NA, nrow(fit_m1) + 5)

quant_vars <- c("adj_pop_density")
order_quant_vars <- c(30)
variable_counts[order_quant_vars] <- ""

qual_vars <- c("age_group", "sex", "race", "adj_perc_bach_quar", "adj_perc_insured_quar")
order_qual_vars <- (1:30)[-order_quant_vars]
...
variable_counts[order_qual_vars] <- calc_freq(all_pcr[, qual_vars])
variable_counts <- c(variable_counts[1:15], 
                     variable_counts[21],
                     variable_counts[16:20],
                     variable_counts[22:30])

table_m1[, 1] <- variable_counts
