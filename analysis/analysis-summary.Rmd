...
load(file = here("analysis", "fit_time_gam_int.Rdata"))
```
...
kable(table_1a, 
      format = "latex", 
      caption = "Cox regression estimates from baseline model of adjusted relative risk of mortality for patients on dialysis in the US with end stage renal disease") %>%
  kable_styling(latex_options = c("HOLD_position"),
                font_size = 11) %>%
  pack_rows("Sex", 2, 3, bold = FALSE) %>%
  pack_rows("Race", 4, 6, bold = FALSE) %>%
  pack_rows("Previous CVD", 8, 9, bold = FALSE) %>%
  pack_rows("Diabetes", 10, 11, bold = FALSE) %>%
  pack_rows("Smoker", 12, 14, bold = FALSE) %>%
  add_footnote(c("Adjusted for all covariates listed", 
                 "Abreviations: CI = confidence interval, ESRD = end stage renal disease, CVD = cardiovascular disease, BMI = body mass index"),
               notation = "symbol")                 
```
