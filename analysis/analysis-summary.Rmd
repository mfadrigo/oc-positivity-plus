...
# 
 ...
 fit_time_gam_summary <- fit_time_gam_info[["model_sum"]]
 fit_time_gam_plot <- fit_time_gam_info[["ci_plot"]]
...
calc_freq <- function(df, order) {
  qual_count <- unlist(apply(df, MARGIN = 2, FUN = table))
  qual_count <- qual_count[order]
...
plot(fit_time_gam_int, scheme = 1)
plot(fit_time_gam_int, scheme = 2)
plot(fit_time_gam_int, scheme = 3)
vis.gam(x = fit_time_gam_int, 
        view = c("time_days", "adj_med_income"),
        plot.type = "persp",
        se = 2)
```
