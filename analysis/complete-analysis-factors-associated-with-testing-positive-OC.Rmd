...
kable(
  ...
  pack_rows("% with Insurance[note]", 28, 31, bold = FALSE) %>% 
...
indent_levels <- function(name) {
  indent <- paste0(rep(" ", 7), collapse = "")
  paste0(indent, name, collapse = "")
}

fit_time_gam_inter_summary1$ind_param_names <- c(
  map_chr(fit_time_gam_inter_summary1$param_names[-27], indent_levels),
  fit_time_gam_inter_summary1$param_names[27]
)
...
ggplot(fm_results_with_ref, aes(x = ind_param_names, y = odds), size = 5) +
  ...
  scale_x_discrete(limits = rev(fm_results_with_ref$ind_param_names)) 
```
