...
if (params$reclean_data) {
  all_pcr_and_zip <- read_all_pcr(
    file_path = here("data", "all-elr-pcr-tests-updated-2020-11-09.csv"),
...
parameter_names <- c(
  "Age [ref: 0-4yrs]   5-9", "10-14", "15-19", "20-24", "25-29", "30-34", 
  "35-39", "40-49", "50-59", "60-69", "70-79", "80+",
  "Sex [ref: Female]   Male",
  "Race [ref: White]   Asian", "Black", "Hispanic", "Native American", "Pacific Islander", "Other", "Unknown",
  "% College Degree [ref: Q1]   2nd Quartile", "3rd Quartile", "4th Quartile",
  "% Insured [ref: Q1]   2nd Quartile ", "3rd Quartile ", "4th Quartile ",
  "Population Density"
  )

fit_time_gam_inter_summary <- compute_ci_gam_logistic(
...
fit_final_model <- fit_time_gam_inter_summary
...
kable(
  ...
  pack_rows("% College Degree[note]", 24, 27, bold = FALSE) %>%
  pack_rows("% Insured[note]", 28, 31, bold = FALSE) %>% 
...
```{r final-model-results-plot1, fig.align="center", fig.cap="Model 5 regression results for linear fixed effects. This plot excludes the coefficient for median income due to the interaction between median income and time. Percent with college degree is the estimated associated change in odds of testing positive for COVID-19 for an individual living in a zip code where the estimate percent of the population with a bachelor's degree is in the 2nd quartile relative to an individual from a zip code in the first. Percent insured representing estimated percent of zip code population with medical insurance is similarly defined."}

ggplot(fit_time_gam_inter_summary, aes(x = param_names, y = odds), size = 5, colour = "red") +
  ylim(min(fit_time_gam_inter_summary$lower_bound), max(fit_time_gam_inter_summary$upper_bound)) +
  geom_point() +
  geom_linerange(aes(ymin = lower_bound, ymax = upper_bound), colour = "black") +
  ...
  scale_x_discrete(limits = rev(parameter_names)) 
```

```{r final-model-results-plot1_alt, fig.align="center", fig.cap="Model 5 regression results for linear fixed effects. This plot excludes the coefficient for median income due to the interaction between median income and time. Percent with college degree is the estimated associated change in odds of testing positive for COVID-19 for an individual living in a zip code where the estimate percent of the population with a bachelor's degree is in the 2nd quartile relative to an individual from a zip code in the first. Percent insured representing estimated percent of zip code population with medical insurance is similarly defined."}
parameter_names <- c(
  "Age [ref: 0-4yrs]                                   5-9", "10-14", "15-19", "20-24", "25-29", "30-34", 
  "35-39", "40-49", "50-59", "60-69", "70-79", "80+",
  "Sex [ref: Female]                                 Male",
...
ggplot(fit_time_gam_inter_summary, aes(x = param_names, y = odds), size = 5, colour = "red") +
  ...
  geom_linerange(aes(ymin = lower_bound, ymax = upper_bound), colour = "black") +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "black") +
  ylab("Odds ratio of testing positive for COVID-19") +
  coord_flip() +
  theme(
    axis.title.y = element_blank(),
    axis.text.x=element_text(size = 12, colour="black"),
...
```{r final-model-results-plot, fig.align="center", fig.cap="Model 5 regression results for linear fixed effects. This plot excludes the coefficient for median income due to the interaction between median income and time. Percent with college degree is the estimated associated change in odds of testing positive for COVID-19 for an individual living in a zip code where the estimate percent of the population with a bachelor's degree is in the 2nd quartile relative to an individual from a zip code in the first. Percent insured representing estimated percent of zip code population with medical insurance is similarly defined.", eval=FALSE}
groups <- c(
  rep("Age [ref: 0-4]", 12), 
  "Sex [ref: Female]", 
  rep("Race [ref: White]", 7),
  rep("POP with Bachelors", 3),
  rep("POP with Insurance", 3),
  " "
)

fm_results <- data.frame(
  fit_time_gam_inter_summary, 
  groups, 
  orders = 1:nrow(fit_time_gam_inter_summary)
)

fm_results <- fm_results %>% 
  mutate(groups = factor(groups, levels = 
    c(
      "Age [ref: 0-4]", 
      "Sex [ref: Female]",
      "Race [ref: White]",
  ...
  mutate(param_names = factor(param_names, levels = 
    c(
      "80+", "70-79", "60-69", "50-59", "40-49", "35-39",
      "30-34", "25-29", "20-24", "15-19", "10-14", "5-9",
      "Male",
      "Unknown", "Other", "Pacific Islander", "Native American", "Hispanic", "Black", "Asian",
...
ggplot(fm_results, aes(x = param_names, y = odds), size = 5, colour = "red") +
  ylim(min(fm_results$lower_bound), max(fm_results$upper_bound)) +
  geom_point() +
